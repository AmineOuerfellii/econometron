...
class VARMA(VAR):
  ...
  def _ini_s1(self, ord=None, output=True, p=None, q=None):
      ...
      if ord is None:
          if self.max_p > 1 or self.max_q > 1:
            ...
            # print('2',cand)
            max_pq = cand if cand < T else min(int(round(np.sqrt(T))), max(1, int(T // (K**2))))
            # print('3',max_pq)
          ...
          for ll in range(1, p_q + 1):
              ...
              for j in range(K):
                  ...
                  if MA[i, idx] > 1:
                      ...
                      X_list.append(tmp)
                      icnt += 1
          X_matrix = np.hstack(X_list) if icnt > 0 else np.ones((Y_i.shape[0], 1))
          beta_a, _, _, diag = ols_estimator(X_matrix, Y_i, add_intercept=False)
  ...
  def _prepare_for_est(self,estimates,stand_err,output=True):
    ...
    if output:
      ...
      print("-" * 55)
      print(f"{'ini_est':^12} | {'lower bound':^12} | {'upperbound':^12} | {'s.err':^12}")
      print("-" * 55)
  ...
  def prepare_A_B_Matrices(self, par, p=None, q=None):
      ...
      if self.structural_id:
          ...
          for i in range(K):
              ...
              if jend > 0:
                  ...
                  for lag in lag_jdx:
                      j = lag % K
                      B[i, lag] = Kpar[icnt]
                      indexes.append((icnt, K + kp + K * (lag // K) + i * K + j))
                      icnt += 1
          assert icnt == len(Kpar), f"Only used {icnt} of {len(Kpar)} parameters."
          ...
          B = Kpar[kp:kp + kq].reshape(K, q * K) if q > 0 else np.zeros((K, 0))
          A = A.reshape(p, K, K)
          B = B.reshape(q, K, K)
          icnt = len(idx_l)
          for lag in range(p):
              for i in range(K):
                  for j in range(K):
                      indexes.append((icnt, K + lag * K * K + i * K + j))  # j encodes A[lag, i, j]
                      icnt += 1
          for lag in range(q):
              for i in range(K):
                  for j in range(K):
                      indexes.append((icnt, K + p * K * K + lag * K * K + i * K + j))  # j encodes B[lag, i, j]
                      icnt += 1
          assert icnt == len(Kpar) + len(idx_l), f"Only used {icnt} of {len(Kpar) + len(idx_l)} parameters."
  ...
  def LL_func(self, par, p=None, q=None,verbose=False):
    ...
    for t in range(T):
        ...
        if ar_lags:
            varma_process += sum(ARs[i,:,:]@Y[:,t-i-1] for i in range(ar_lags))
        if ma_lags:
            varma_process += sum(MAs[j,:,:]@Eps[:,t-j-1] for j in range(ma_lags))
    ...
    try:
      ...
      sign, logdet = np.linalg.slogdet(Sigma)
      if verbose: print("Sigma singular -> penalty")
      return 1e7
    eff_t=resid.shape[1]
    ll=-(eff_t*K)/2*np.log(2*np.pi)-(eff_t)/2*logdet-0.5*np.trace(resid.T @ inv_Sigma @ resid)
    neg_ll=-ll
    return neg_ll
  def _build_companion(self, matrix, K, order):
    ...
    if not isinstance(matrix, np.ndarray) or matrix.shape != (order, K, K):
        raise ValueError(f"Expected matrix shape ({order}, {K}, {K}), got {matrix.shape}")
    comp_dim = K * order
    ...
    if order == 1:
        ...
        bottom = np.eye(K * (order - 1), K * order) if order > 1 else np.zeros((0, comp_dim))
        companion = np.vstack([top, bottom])
        if companion.shape[0] != companion.shape[1]:
            raise ValueError(f"Companion matrix is not square: shape {companion.shape}")
        return companion
  ...
  def compute_diags(self, gs=False, model=None):
      if gs:
          if model is None:
              raise ValueError("For grid search mode, model must be provided.")
          target = model
      else:
      ...
      target.update(diags)
      if len(se) != len(par):
          print(f"Warning: Diagnostics length ({len(se)}) does not match parameters ({len(par)})")
      return diags
  ...
  def display_results(self, results):
      ...
      # === HEADER ===
      print("=" * 70)
      print(f"{'Structural' if self.structural_id else 'Non-Structural'} VARMA({p},{q})".center(70))
      print("=" * 70)
      print(f"Log-likelihood: {loglik:.4f} | Model type: {model_type} | Time: {time_now}")
      print(f"AIC: {aic:.4f} | BIC: {bic:.4f} | HQIC: {hqic:.4f} | Hannan-Rissanen Method")
      print("=" * 70)
      ...
      if self.structural_id and hasattr(self, "Ph0"):
          ...
          if np.any(np.abs(self.Ph0 - np.eye(K)) > 1e-10):
              df_ph0 = pd.DataFrame(self.Ph0, columns=[f"Y{j+1}" for j in range(K)],index=[f"Y{i+1}" for i in range(K)])
  ...
  def fit(self, p=None, q=None, plot=True, verbose=True, enforce_stab_inver=False):
      ...
      if self.structural_id:
          ...
          if p is None or q is None:
              grid_search = True
              if verbose:
                  logger.info(f"[INFO] Falling back to grid search over p=0 to {self.max_p}, q=0 to {self.max_q}")
          else:
              if verbose:
                  logger.info(f"[INFO] Using non-structural VARMA, p={p}, q={q}")
      T, K = self.data.shape
      crit_name = self.criterion.lower()
      if grid_search:
          results = []
          pq_pairs = [(i, j) for i in range(self.max_p + 1) for j in range(self.max_q + 1)]
          for pp, qq in pq_pairs:
              if pp == 0 and qq == 0:
                  ...
                  continue
              try:
                  estimates, stand_err = self._ini_s1(p=pp, q=qq, output=False)
                  if len(estimates) == 0:
                      continue
                  par, separ, lower_bounds, upper_bounds = self._prepare_for_est(estimates, stand_err, output=False)
                  if len(par) == 0:
                      continue
                  constraints = []
                  ...
                  result = minimize(self.LL_func,par,args=(pp, qq, False),method='trust-constr',bounds=list(zip(lower_bounds, upper_bounds)),constraints=constraints,options={'disp': False, 'maxiter': 500, 'gtol': 1e-6, 'xtol': 1e-6})
                  if not result.success or np.isnan(result.fun) or np.isinf(result.fun):
                      if verbose:
                          logger.warning(f"[WARN] Optimization failed for p={pp}, q={qq}: fun={result.fun}")
                      continue
                  A, B, Cst,indexes = self.prepare_A_B_Matrices(result.x, p=pp, q=qq)
                  resids = self.get_resids(A, B, Cst)
                  cr = self.get_crit(resids, len(result.x), crit_name)
                  if np.isnan(cr) or np.isinf(cr):
                      continue
                  model = {'par': result.x,'A': A,'B': B,'p': pp,'q': qq,'Cst': Cst,crit_name: cr,'residuals': resids,'loglikelihood': -result.fun,'Index': indexes}
                  results.append(model)
              except Exception as e:
                  if verbose:
                      logger.warning(f"[WARN] Exception for p={pp}, q={qq}: {str(e)}")
                  continue
          if not results:
          ...
          try:
            if ini_p==0 and ini_q==0:
              ...
              estimates, stand_err = self._ini_s1(ord=None, output=verbose, p=ini_p, q=ini_q)
              par, separ, lower_bounds, upper_bounds = self._prepare_for_est(estimates, stand_err, output=verbose)
              constraints = []
              if enforce_stab_inver:
                  constraints = [{
                          'type': 'ineq',
              ...
              if not result.success or np.isnan(result.fun) or np.isinf(result.fun):
                  raise ValueError(f"Optimization failed: {result.message}, fun={result.fun}")
              A, B, Cst ,indexes = self.prepare_A_B_Matrices(result.x, p=ini_p, q=ini_q)
              resids = self.get_resids(A, B, Cst)
              cr = self.get_crit(resids, len(result.x), crit_name)
              p=self.best_p if self.structural_id else ini_p
              q=self.best_q if self.structural_id else ini_q
              self.best_model = {'par': result.x,'A': A,'B': B,'p': p,'q': q,'Cst': Cst,crit_name: cr,'residuals': resids,'loglikelihood': -result.fun,'Index': indexes}
          except Exception as e:
              raise ValueError(f"Single model fit failed: {str(e)}")
      self.compute_diags()
      if verbose:
          print(f"Model fitted with p={self.best_model['p']}, q={self.best_model['q']}, Log-likelihood: {self.best_model['loglikelihood']:.4f}")
  ...
  def run_full_diagnosis(self, num_lags=8, plot=False, threshold=None):
      ...
      if resids.ndim != 2 or resids.shape[1] != K:
          raise ValueError(f"Residuals shape mismatch. Expected ({T},{K}), got {resids.shape}")
      ...
      if p > 0:
        ...
        print(f"The VARMA AR-part is {'stable' if is_stable else 'non-stable'}")
      else:
          is_stable = True
      ...
      if q > 0:
          ...
          if ma_comp_dim == K:
              ...
              ma_bottom = np.eye(K * (q - 1), K * q) if q > 1 else np.zeros((0, ma_comp_dim))
          ...
          print(f"The VARMA MA-part is {'invertible' if is_invertible else 'non-invertible'}")
      else:
          is_invertible = True
      ...
      for i in range(K):
          ...
          print(f"Residual {i} ({self.columns[i]}): ARCH p-value = {pval:.4f} → {arch_result}")
      ...
      for i in range(K):
          ...
          try:
              ...
              sh_pval = 0.0
          print(f"Residual {i} ({self.columns[i]}): JB p-value = {jb_pval:.4f}, Shapiro p-value = {sh_pval:.4f}")
      ...
      final_score = (S_score + I_score+auto_corr_score +arch_score +normality_score+structural_breaks_score )/5                         
      self.fitted = final_score >= threshold
      Diagnosis['Final_score'] = final_score
      Diagnosis['Verdict'] = 'Passed' if self.fitted else 'Failed'
      print("\\n==================Diagnostic Summary=================")
      q = self.best_model.get('B').shape[0] if self.best_model.get('B') is not None else 0
      summary_table = {
      ...
      if plot:
          ...
          for i in range(K):
              ...
              ax_hist.hist(resids[:, i], bins=30, density=True, alpha=0.7, color='skyblue')
  ...
  def predict(self, n_periods=10, plot=True):
      ...
      A = np.array(self.best_model.get('A', np.zeros((0, self.data.shape[1], self.data.shape[1]))))
      B = np.array(self.best_model.get('B', np.zeros((0, self.data.shape[1], self.data.shape[1]))))
      Cst = np.ravel(self.best_model.get('Cst', np.zeros((self.data.shape[1],))))
      ...
      resid_buffer = [resids[:, -i - 1].copy() for i in range(q)] if q > 0 else []
      ...
      if isinstance(self.data.index, pd.DatetimeIndex):
          freq = self.data.index.freq or pd.infer_freq(self.data.index) or 'D'
          start = self.data.index[-1]
          forecast_index = pd.date_range(start=start + (self.data.index.freq or pd.Timedelta(days=1)),periods=n_periods,freq=freq)
      else:
          forecast_index = range(len(self.data), len(self.data) + n_periods)
      forecast_df = pd.DataFrame(forecasts, index=forecast_index, columns=self.columns)
      ci_lower_df = pd.DataFrame(ci_lower, index=forecast_index, columns=self.columns)
      ci_upper_df = pd.DataFrame(ci_upper, index=forecast_index, columns=self.columns)
      if plot:
          T = len(self.data)
          ...
          for i, col in enumerate(self.columns):
              ...
              ax.plot(time_range[:T], whole[col].iloc[:T], 'b-', label='Historical', linewidth=1.5)
  ...
  def simulate(self, n_periods=100, plot=True):
      if self.best_model is None:
          raise ValueError("No fitted model available. Fit model before simulate().")
      A = np.array(self.best_model.get('A', np.zeros((0, self.data.shape[1], self.data.shape[1]))))
      B = np.array(self.best_model.get('B', np.zeros((0, self.data.shape[1], self.data.shape[1]))))
      Cst = np.ravel(self.best_model.get('Cst', np.zeros((self.data.shape[1],))))
      p = self.best_model.get('p', A.shape[0]) if A.size else 0
      q = self.best_model.get('q', B.shape[0]) if B.size else 0
      K = self.data.shape[1]
      Y_hist = [self.data.to_numpy()[-i - 1].copy() for i in range(p)] if p > 0 else []
      resids = self.best_model.get('residuals', np.zeros((K, 0)))
      ...
      for t in range(n_periods):
          ...
          y_pred = Cst.copy()
          for i in range(p):
              y_pred += A[i] @ (Y_hist[i] if i < len(Y_hist) else np.zeros(K))
          for j in range(q):
              y_pred += B[j] @ (E_hist[j] if j < len(E_hist) else np.zeros(K))
          y_t = y_pred + eps_t
          Y_sim[t, :] = y_t
          if p > 0:
              Y_hist = [y_t] + Y_hist[:-1] if len(Y_hist) >= p else [y_t] + Y_hist
          if q > 0:
              E_hist = [eps_t] + E_hist[:-1] if len(E_hist) >= q else [eps_t] + E_hist
      if plot:
          fig, axes = plt.subplots(K, 1, figsize=(10, 4 * K), sharex=True)
          axes = [axes] if K == 1 else axes
          for i in range(K):
              axes[i].plot(Y_sim[:, i], label=f'Simulated {self.data.columns[i]}')
              axes[i].set_title(f'Simulated Series for {self.data.columns[i]}')
  ...
  def impulse_res(self, h=10, bootstrap=True, n_boot=1000, plot=True):
      if self.best_model is None:
          raise ValueError("No fitted model available. Fit model before impulse_res().")
      A = np.array(self.best_model.get('A', np.zeros((0, self.data.shape[1], self.data.shape[1]))))
      B = np.array(self.best_model.get('B', np.zeros((0, self.data.shape[1], self.data.shape[1]))))
      resids = self.best_model.get('residuals', np.zeros((self.data.shape[1], 0)))
      K = self.data.shape[1]
      p = self.best_model.get('p', A.shape[0]) if A.size else 0
      q = self.best_model.get('q', B.shape[0]) if B.size else 0
          ...
          for i in range(K):
              for j in range(K):
                  ...
                  if 'ci_lower' in result and 'ci_upper' in result:
                      axes[idx].fill_between(range(h + 1), result['ci_lower'][:, i, j], result['ci_upper'][:, i, j],alpha=0.3, color='red', label='95% CI')
                  axes[idx].set_title(f'{self.data.columns[i]} response to {self.data.columns[j]} shock')
  ...
  def FEVD(self, h=10, plot=True):
      if self.best_model is None:
          raise ValueError("No fitted model available. Fit model before FEVD().")
      K = self.data.shape[1] 
      ...
      for i in range(h):
          ...
          for j in range(K): 
              ...
              for k in range(K): 
                  ...
                  fevd[i, j, k] = contribution / mse[i, j] if mse[i, j] != 0 else 0
          for j in range(K):
      ...
      if plot:
          ...
          for j in range(K):
              bottom = np.zeros(h)
              for k in range(K):
                  axes[j].bar(range(1, h + 1), fevd[:, j, k], bottom=bottom,label=f'Shock from {self.data.columns[k]}')
                  bottom += fevd[:, j, k]
