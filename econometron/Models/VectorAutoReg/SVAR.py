...
class SVAR(VAR):
    ...
    def identify(self, method='chol', A=None, B=None):
        ...
        if self.best_model is None:
            raise ValueError("No model fitted. Cannot perform structural identification.")
        
        K = len(self.columns)
        Sigma = np.cov(self.best_model['residuals'].T)
        
        if method not in ['chol', 'blanchard-quah', 'AB']:
            raise ValueError("Identification method must be 'chol', 'blanchard-quah', or 'AB'")
        ...
        if method == 'chol':
            ...
            irf = self.impulse_res(h=50, orth=False, bootstrap=False, plot=False)
            ...
            if A is None or B is None:
                raise ValueError("A and B matrices must be provided for AB identification")
            if A.shape != (K, K) or B.shape != (K, K):
                raise ValueError("A and B must be square matrices of size K x K")
            self.A = A
            self.B = B
            ...
            if not np.allclose(self.A @ Sigma @ self.A.T, self.B @ self.B.T):
                raise ValueError("AB model does not satisfy identification condition")
    ...
    def _orthogonalize(self, Sigma):
        ...
        if self.A_inv_B is None:
            raise ValueError("Structural identification not performed. Run identify() first.")
        return self.A_inv_B
            ...
            if orth:
                ...
                try:
                    if self.identification_method == 'chol':
                        ...
                        boot_irf_long = np.array([boot_Psi[i] for i in range(50)])
    ...
    def FEVD(self, h=10, plot=False):
        ...
        for i in range(h):
            for j in range(K):
                ...
                for k in range(K):
                    fevd[i, j, k] = np.sum(irf[:i + 1, j, k] ** 2 * (self.B @ self.B.T)[k, k]) / mse[i, j] if mse[i, j] != 0 else 0
    ...
    def get_structural_shocks(self):
        ...
        if self.A is None or self.best_model is None:
            raise ValueError("Structural identification not performed or model not fitted.")
    ...
    def shock_decomposition(self, h=10, plot=False):
        ...
        if self.best_model is None or self.A_inv_B is None:
            raise ValueError("Model not fitted or structural identification not performed.")
        ...
        if plot:
            ...
            for j in range(K):
                ...
                for k in range(K + 1):
                    ...
                    axes[j].plot(range(T), self.data[self.columns[j]], 'k--', label='Actual Data')
